...
"""
Name:    99_same_p0_appearance.py
Purpose: check whether p0 values occur regionally at the same time
...
savepath = r'D:\Netatmo_5min\03_results\99_same_p0_appearance'
...
p0_indi = np.zeros((dwd.root.name.shape[0], 5136))
...
for dwd_id, dwd_name in enumerate(dwd.root.name.read()):
    ...
    dwd_indi_p0 = np.zeros_like(dwd_data)
    dwd_indi_p0[dwd_data != 0] = 1

    p0_indi[dwd_id, :] = dwd_indi_p0

p0_indi_sum = np.sum(p0_indi, axis=0)
p0_idx = np.where(p0_indi_sum > 0)[0]
...
mail.status_mail(
    subject='99_same_p0_appearance.py finished %s' % datetime.datetime.now().strftime(
        '%Y-%m-%d %H:%M:%S'))
